...
class KMeans(object):
  ...
  def train(self, X, epsilon=1e-12):
    ...
    while (all(num > epsilon for num in change)):
        ...
        assign = np.zeros(dists_shape[0])
        centroids_sum = np.zeros_like(self.centroids)
        centroids_count = np.zeros(self.centroids.shape[0])

        for i in range(dists_shape[0]):
          assign[i] = np.argmin(dists[i])
          index = int(assign[i])
          centroids_sum[index] += X[i]
        ...
        for i in range(self.centroids.shape[0]):
          if centroids_count[i] == 0:
            pass
          new_centroids[i] = centroids_sum[i]/centroids_count[i]
